...
**Current Stance**
公式：[立场] + [判断依据/evidence] + [对竞争对手/现状的态度] + [与其他 stakeholder 的关键关系]
立场：Supportive / Neutral / Cautious / Against / Unknown
- ❌ "Supportive"
- ✅ "偏正面但尚未公开支持 — 在我们的 Well-Architected Review 后对30%成本优化感兴趣，但还没内部为我们说话。2年前主导了 Azure 迁移决策，切换有政治风险。与 VP Eng 是强联盟（前公司同事），CTO 高度信任他的技术判断。"
...
<!-- AGENT GUIDANCE:
这是从现在到 close 的完整路线图。每行 = 一个里程碑。

写作标准：
- Target Window 从 Deal Objective close date 倒推
- Milestone 必须是 outcome-oriented + buyer-verifiable，不是活动描述
  - ❌ "跟 CTO 开会"（活动，seller-only）
  - ❌ "发送 proposal"（活动，seller-only）
  - ✅ "CTO 确认目标架构方案并授权启动 POC"（结果，有客户动作）
  - ✅ "EB + Champion live review proposal，确认进入采购流程"（结果，有客户动作）
- 每个 milestone 必须包含客户动作/确认 — 如果只有 AWS 在做事，deal 没在推进
- Milestone 类型不只是 meeting：
  - Meeting / Call（常规拜访）
  - Workshop / POC（技术验证）
...
- AWS Team 根据阶段匹配合适资源：
  - 早期 Discovery: AM + SA
  - 技术验证: SA + Specialist SA (AI/ML, Security, Data...)
  - POC 执行: SA + ProServ/Partner
  - 高管 engage: AM + Executive Sponsor
  - 商务谈判: AM + Deal Desk + Legal
  - 实施: SA + TAM + CSM + ProServ

Exit Criteria 自检（不出现在最终文档，agent 内部把关）：
- 每个 milestone 写完后自问："达成什么算完成？evidence 是什么？"
...
| # | Target Window | Milestone | Key Stakeholders | AWS Team | Status |
|---|---|---|---|---|---|
| 1 | `{Week 1-2}` | `{e.g., CTO + IT Director 确认业务痛点和决策流程，授权进入技术评估}` | `{CTO, IT Director}` | `{AM, SA}` | **Next** ↓ |
| 2 | `{Week 3-4}` | `{e.g., 架构评审完成，CTO 签字确认目标架构方案，同意启动 POC}` | `{CTO, Lead Architect}` | `{SA, Data Specialist}` | Planned |
| 3 | `{Week 5-7}` | `{e.g., POC scorecard 全绿，VP Eng 确认技术满足需求，提交 business case}` | `{VP Eng, CTO}` | `{SA, ProServ}` | Planned |
| 4 | `{Week 8-9}` | `{e.g., CFO approve business case，确认预算，指示采购启动流程}` | `{CFO, Champion}` | `{AM, Deal Desk}` | Planned |
| 5 | `{Week 10-12}` | `{e.g., 合同条款谈妥，EB 签署协议，implementation kickoff scheduled}` | `{Procurement, Legal, EB}` | `{AM, Legal}` | Planned |
