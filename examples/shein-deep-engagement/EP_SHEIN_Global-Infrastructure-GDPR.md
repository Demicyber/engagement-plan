...
*Engagement sequence rationale: 张锐（Technical Champion）已经在我们这边，持续用他推动内部进程。王凯（CTO/Economic Buyer）是最终决策人，需要在这次会议确认 timeline。李莎（Compliance Gatekeeper）是当前 blocker — 她的 reference case 需求必须在这次会议给出明确回应，否则 timeline 会被拖延。策略：先满足李莎的合规关切 → 解除 blocker → 王凯才能 commit timeline。*
...
|---|---|---|---|---|
| 1 | Week 1 (Mar 5) | First meeting: validate pain, understand multi-cloud challenges, confirm opportunity scope | CTO 王凯, Head of Infra 张锐 | **Done** |
| 2 | Week 3 (Mar 19) | CDN + Recommendation deep-dive, benchmark AWS vs current setup | CTO 王凯, 张锐, Infra team | **Done** |
| 3 | Week 6 (Apr 9) | GDPR architecture workshop + Security deep-dive, introduce 李莎 | CTO 王凯, 张锐, 李莎 | **Done** |
| 4 | Week 9 (Apr 23) | Technical validation complete; DPA review with legal; confirm deal structure | CTO 王凯, 张锐, 李莎 | **Done** |
| 5 | Week 12 (May 21) | Push for timeline commitment; address compliance blockers; competitive response | CTO 王凯, 张锐, 李莎 | **Next** ↓ |
| 6 | Week 14 (Jun 4) | Reference call (ASOS/EU customer) + final compliance sign-off | 李莎 + legal team | Planned |
| 7 | Week 16 (Jun 18) | Commercial negotiation: PPA terms, pricing, payment structure | CTO 王凯, Procurement | Planned |
| 8 | Week 18-19 (Jul) | Contract execution + implementation kickoff | CTO 王凯, 张锐, Procurement | Planned |
...
- **CTO 王凯** — commit to PPA signing timeline (target: contract execution by end of June); confirm Google Cloud is not being awarded GDPR workload separately
...
|---|---|---|---|
| 2026-04-23 | #4 | Technical validation 完成。张锐 sign off CDN + 推荐方案。李莎提出 reference case 需求和 DPA 修改请求。王凯首次透露 Google Cloud 竞争（"给我一个理由不选他们做 GDPR"）。 | Technical Validation → Business Validation |
...
| 2026-03-19 | #2 | CDN benchmark 完成：CloudFront vs 现有方案 latency 降低 23%（亚太）。推荐系统 POC scope 确认。王凯 stance 从 Neutral 转向 Supportive。张锐开始直接跟 SA 李珊 WeChat 沟通。 | Prospect → Qualified |
| 2026-03-05 | #1 | 首次拜访成功。痛点验证：3 朵云运维复杂度 + GDPR 合规压力。王凯确认机会真实性。张锐表达对 AWS CDN 的强烈兴趣。Opportunity scope 从最初的 CDN-only 扩展到包含推荐和 GDPR。 | — (Prospect) |
